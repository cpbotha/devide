...
class PyAUIFrame(wx.Frame):
    ...
    def __init__(self, parent, id=-1, title="", pos=wx.DefaultPosition,
        ...
        self._mgr.AddPane(self.CreateGrid(), PyAUI.PaneInfo().Name("grid_content").
                          CenterPane().Hide())

        self._mgr.AddPane(self.CreateTreeCtrl(), PyAUI.PaneInfo().Name("tree_content").
                          CenterPane().Hide())
                      
        self._mgr.AddPane(self.CreateSizeReportCtrl(), PyAUI.PaneInfo().Name("sizereport_content").
                          CenterPane().Hide())

        self._mgr.AddPane(self.CreateTextCtrl(), PyAUI.PaneInfo().Name("text_content").
                          CenterPane().Hide())

        self._mgr.AddPane(self.CreateHTMLCtrl(), PyAUI.PaneInfo().Name("html_content").
                          CenterPane())
                                
        # add the toolbars to the manager
                        
        self._mgr.AddPane(tb1, PyAUI.PaneInfo().
                          Name("tb1").Caption("Big Toolbar").
                          ToolbarPane().Top().
                          LeftDockable(False).RightDockable(False))

        self._mgr.AddPane(tb2, PyAUI.PaneInfo().
    ...
    def OnAbout(self, event):
        ...
        msg = "This Is The About Dialog Of The PyAUI Demo.\n\n" + \
              "Author: Andrea Gavana @ 02 Jan 2006\n\n" + \
        ...
        dlg = wx.MessageDialog(self, msg, "PyAUI wxPython Demo",
    ...
    def OnCreateHTML(self, event):
        ...
        self._mgr.AddPane(self.CreateHTMLCtrl(), PyAUI.PaneInfo().
    ...
    def OnChangeContentPane(self, event):
        ...
        self._mgr.GetPane("tree_content").Show((event.GetId() == ID_TreeContent \
                                                and [True] or [False])[0])
        self._mgr.GetPane("sizereport_content").Show((event.GetId() == \
                                                      ID_SizeReportContent and \
                                                      [True] or [False])[0])
    ...
    def GetIntroText(self):
        ...
        text = \
        "<html><body>" \
        "<h3>Welcome to PyAUI 0.9.2</h3>" \
        "<br/><b>Overview</b><br/>" \
        "<p>PyAUI is an Advanced User Interface library for the wxWidgets toolkit " \
        "that allows developers to create high-quality, cross-platform user " \
        "interfaces quickly and easily. The wxPython version has been converted " \
        "from the original C++ wxWidgets implementation by Andrea Gavana, " \
        "@ 24-Dec-2005.</p>" \
        "<p><b>Features</b></p>" \
        "<p>With PyAUI version 0.9.2, developers can create application frameworks with:</p>" \
        "<ul>" \
        ...
        "<p>The following features/fixes have been added since the last version of PyAUI:</p>" \
        "<ul>" \
        ...
        "<p>The following features have been added PyAUI 0.9.1:</p>" \
        "<ul>" \
        ...
        "<li>Support for vertical toolbars</li>" \
...
class SettingsPanel(wx.Panel):
    ...
    def UpdateColors(self):
        ...
        bk = self._frame.GetDockArt().GetColor(PyAUI.AUI_ART_BACKGROUND_COLOUR)
        self._background_color.SetBitmapLabel(self.CreateColorBitmap(bk))
        
        cap = self._frame.GetDockArt().GetColor(PyAUI.AUI_ART_INACTIVE_CAPTION_COLOUR)
        self._inactive_caption_color.SetBitmapLabel(self.CreateColorBitmap(cap))
        
        capgrad = self._frame.GetDockArt().GetColor(PyAUI.AUI_ART_INACTIVE_CAPTION_GRADIENT_COLOUR)
        self._inactive_caption_gradient_color.SetBitmapLabel(self.CreateColorBitmap(capgrad))
        
        captxt = self._frame.GetDockArt().GetColor(PyAUI.AUI_ART_INACTIVE_CAPTION_TEXT_COLOUR)
        self._inactive_caption_text_color.SetBitmapLabel(self.CreateColorBitmap(captxt))
        
        acap = self._frame.GetDockArt().GetColor(PyAUI.AUI_ART_ACTIVE_CAPTION_COLOUR)
        self._active_caption_color.SetBitmapLabel(self.CreateColorBitmap(acap))
        
        acapgrad = self._frame.GetDockArt().GetColor(PyAUI.AUI_ART_ACTIVE_CAPTION_GRADIENT_COLOUR)
        self._active_caption_gradient_color.SetBitmapLabel(self.CreateColorBitmap(acapgrad))
        
        acaptxt = self._frame.GetDockArt().GetColor(PyAUI.AUI_ART_ACTIVE_CAPTION_TEXT_COLOUR)
        self._active_caption_text_color.SetBitmapLabel(self.CreateColorBitmap(acaptxt))
        
        sash = self._frame.GetDockArt().GetColor(PyAUI.AUI_ART_SASH_COLOUR)
        self._sash_color.SetBitmapLabel(self.CreateColorBitmap(sash))
        
        border = self._frame.GetDockArt().GetColor(PyAUI.AUI_ART_BORDER_COLOUR)
        self._border_color.SetBitmapLabel(self.CreateColorBitmap(border))
        
        gripper = self._frame.GetDockArt().GetColor(PyAUI.AUI_ART_GRIPPER_COLOUR)
        self._gripper_color.SetBitmapLabel(self.CreateColorBitmap(gripper))
    
    
    def OnPaneBorderSize(self, event):
    
        self._frame.GetDockArt().SetMetric(PyAUI.AUI_ART_PANE_BORDER_SIZE,
                                           event.GetInt())
    ...
    def OnSashSize(self, event):
        ...
        self._frame.GetDockArt().SetMetric(PyAUI.AUI_ART_SASH_SIZE,
                                           event.GetInt())
        self._frame.DoUpdate()
    ...
    def OnSetColor(self, event):
        ...
        if event.GetId() == ID_BackgroundColor:
            var = PyAUI.AUI_ART_BACKGROUND_COLOUR
        elif event.GetId() == ID_SashColor:
            var = PyAUI.AUI_ART_SASH_COLOUR
        elif event.GetId() == ID_InactiveCaptionColor:
            var = PyAUI.AUI_ART_INACTIVE_CAPTION_COLOUR
        elif event.GetId() == ID_InactiveCaptionGradientColor:
            var = PyAUI.AUI_ART_INACTIVE_CAPTION_GRADIENT_COLOUR
        elif event.GetId() == ID_InactiveCaptionTextColor:
            var = PyAUI.AUI_ART_INACTIVE_CAPTION_TEXT_COLOUR
        elif event.GetId() == ID_ActiveCaptionColor:
            var = PyAUI.AUI_ART_ACTIVE_CAPTION_COLOUR
        elif event.GetId() == ID_ActiveCaptionGradientColor:
            var = PyAUI.AUI_ART_ACTIVE_CAPTION_GRADIENT_COLOUR
        elif event.GetId() == ID_ActiveCaptionTextColor:
            var = PyAUI.AUI_ART_ACTIVE_CAPTION_TEXT_COLOUR
        elif event.GetId() == ID_BorderColor:
            var = PyAUI.AUI_ART_BORDER_COLOUR
        elif event.GetId() == ID_GripperColor:
            var = PyAUI.AUI_ART_GRIPPER_COLOUR
        else:
            return        
...
def main():
    ...
    frame = PyAUIFrame(None, wx.ID_ANY, "PyAUI wxPython Demo ;-)",
