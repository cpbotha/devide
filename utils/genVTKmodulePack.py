...
# module_name, vtkObjectName, progressText, inputDescriptions, outputDescrs
...
def createDeVIDEModuleFromVTKObject(vtkObjName):
    ...
    if vtkObjName.endswith('Writer'):
        module_name = vtkObjName

        # with module_name[:-6] we snip off the 'Writer'
        moduleText = moduleSkeleton1 % (module_name, vtkObjName,
                                        'Writing %s.' % (module_name[:-6],),
                                        (vtkObjName[:-6],), (),
                                        None, None)
        return (module_name, moduleText, 'VTK basic writer', vtkObj.__doc__)
        
    elif vtkObjName.endswith('Reader'):
        module_name = vtkObjName

        # with module_name[:-6] we snip off the 'Reader'
        moduleText = moduleSkeleton1 % (module_name, vtkObjName,
                                        'Reading %s.' % (module_name[:-6],),
                                        (), (vtkObjName[:-6],),
                                        None, None)
        return (module_name, moduleText, 'VTK basic readers', vtkObj.__doc__)
        ...
        return (module_name, moduleText, 'VTK basic filters', vtkObj.__doc__)
...
def main():
    ...
    for vtkObjName in list2:
        module_name, moduleText, moduleCats, module_doc = \
        ...
        if module_name:
            ...
            moduleListStrings.append("class %s:" % (module_name,))
