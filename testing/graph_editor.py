...
class GraphEditorTestBase(unittest.TestCase):
    def setUp(self):
        ...
        self._devide_app.get_interface()._handlerMenuGraphEditor(None)
        # make sure we begin with a clean slate, so we can do
        # some module counting
        self._devide_app.get_interface()._graphEditor.clearAllGlyphsFromCanvas()
...
class GraphEditorVolumeTestBase(GraphEditorTestBase):
    ...
    def setUp(self):
        ...
        (sqmod, sqglyph) = self._devide_app.get_interface()._graphEditor.createModuleAndGlyph(
            10, 10, 'modules.Misc.superQuadric')

        (ivmod, ivglyph) = self._devide_app.get_interface()._graphEditor.createModuleAndGlyph(
            10, 70, 'modules.Misc.implicitToVolume')

        (dtmod, dtglyph) = self._devide_app.get_interface()._graphEditor.createModuleAndGlyph(
        ...
        ret = self._devide_app.get_interface()._graphEditor._connect(sqglyph, 0, ivglyph, 0)
        ret = self._devide_app.get_interface()._graphEditor._connect(ivglyph, 0, dtglyph, 0)

        # redraw
        self._devide_app.get_interface()._graphEditor._graphEditorFrame.canvas.redraw()
...
class GraphEditorBasic(GraphEditorTestBase):
    ...
    def test_startup(self):
        ...
        """
        self.failUnless(
           self._devide_app.get_interface()._graphEditor._graphEditorFrame.IsShown())

    def test_module_creation_deletion(self):
        """Creation of simple module and glyph.
        """

        (mod, glyph) = self._devide_app.get_interface()._graphEditor.createModuleAndGlyph(
            10, 10, 'modules.misc.superQuadric')
        self.failUnless(mod and glyph)

        ret = self._devide_app.get_interface()._graphEditor._deleteModule(glyph)
        self.failUnless(ret)
    ...
    def test_module_help(self):
        ...
        self._devide_app.get_interface()._graphEditor._helpModule(mod)

        # find frame that should have appeared by now
        fullModuleName = mod.__class__.__module__
        htmlWindowFrame = self._devide_app.get_interface()._graphEditor._moduleHelpFrames[
            fullModuleName]
    ...
    def test_simple_network(self):
        ...
        (svmod, svglyph) = self._devide_app.get_interface()._graphEditor.createModuleAndGlyph(
            10, 90, 'modules.viewers.slice3dVWR')

        ret = self._devide_app.get_interface()._graphEditor._connect(sqglyph, 1, svglyph, 0)
        self._devide_app.get_interface()._graphEditor._graphEditorFrame.canvas.redraw()
        
        self.failUnless(ret)

        # now run the network
        self._devide_app.get_interface()._graphEditor._handler_execute_network(None)
    ...
    def test_config_vtk_obj(self):
        ...
        self._devide_app.get_interface()._graphEditor._viewConfModule(sqmod)
        ...
        ret = self._devide_app.get_interface()._graphEditor._deleteModule(sqglyph)
        self.failUnless(ret)
...
class TestModulesMisc(GraphEditorTestBase):
    ...
    def test_create_destroy(self):
        ...
        for module_name in ml:
            ...
            # destroy
            ret = self._devide_app.get_interface()._graphEditor._deleteModule(
                cglyph)
...
class TestITKBasic(GraphEditorVolumeTestBase):
    ...
    def test_confidence_seed_connect(self):
        ...
        ret = self._devide_app.get_interface()._graphEditor._connect(self.dtglyph, 0, svglyph, 0)
        ...
        (v2imod, v2iglyph) = self._devide_app.get_interface()._graphEditor.createModuleAndGlyph(
            200, 10, 'modules.Insight.VTKtoITKF3')

        (cscmod, cscglyph) = self._devide_app.get_interface()._graphEditor.createModuleAndGlyph(
            200, 70, 'modules.Insight.confidenceSeedConnect')

        (i2vmod, i2vglyph) = self._devide_app.get_interface()._graphEditor.createModuleAndGlyph(
            200, 130, 'modules.Insight.ITKF3toVTK')

        ret = self._devide_app.get_interface()._graphEditor._connect(self.dtglyph, 0, v2iglyph, 0)
        self.failUnless(ret)
        
        ret = self._devide_app.get_interface()._graphEditor._connect(v2iglyph, 0, cscglyph, 0)
        self.failUnless(ret)
        
        ret = self._devide_app.get_interface()._graphEditor._connect(cscglyph, 0, i2vglyph, 0)
        self.failUnless(ret)

        # there's already something on the 0'th input of the slice3dVWR
        ret = self._devide_app.get_interface()._graphEditor._connect(i2vglyph, 0, svglyph, 1)
        self.failUnless(ret)
        
        # connect up the selected points
        ret = self._devide_app.get_interface()._graphEditor._connect(svglyph, 0, cscglyph, 1)
        self.failUnless(ret)        
